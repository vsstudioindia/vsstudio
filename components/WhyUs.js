...
/* ── Value card ──────────────────────────────────────────────────────────── */
function ValueCard({ num, title, body, tag, revealDelay }) {
  ...
  return (
    <div
      ...
      style={{
        position:   'relative',
        background: hovered ? '#111' : 'var(--black2)',
        padding:    '44px 40px',
        overflow:   'hidden',
        ...
        transitionDuration:        `0.3s, 0.8s, 0.8s`,
        transitionTimingFunction:  'ease, ease, ease',
        transitionDelay:           `0s, ${revealDelay}ms, ${revealDelay}ms`,
      }}
    >
...
/* ── Stat card ───────────────────────────────────────────────────────────── */
function StatCard({ value, label, revealDelay }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: `opacity 0.8s ease ${revealDelay}ms, transform 0.8s ease ${revealDelay}ms`,
      }}
    >
  ...
  return (
    <section id="planners" style={{ background: 'var(--black)' }}>
      ...
      >
        ...
        >
          <div style={{ width: '32px', height: '1px', background: 'var(--gold)', opacity: 0.5, flexShrink: 0 }} />
          <p
            style={{
              fontFamily:    "'Montserrat', sans-serif",
              fontSize:      '0.58rem',
        ...
        {/* Two-column: headline | premise + CTA */}
        <div
          style={{
            display:             'grid',
      ...
      >
        {VALUE_CARDS.map((card, i) => (
          <ValueCard key={card.num} {...card} revealDelay={i * 80} />
        ))}
      </div>
      ...
      >
        {STATS.map((stat, i) => (
          <StatCard key={stat.value} {...stat} revealDelay={i * 80} />
        ))}
      </div>
      ...
      >
        ...
        >
          "When you hire VS Studio, your clients don't just remember their wedding. They{' '}
          <em style={{ color: 'var(--gold)', fontStyle: 'inherit' }}>relive</em>
